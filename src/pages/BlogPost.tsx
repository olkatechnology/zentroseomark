...
const BlogPostPage = () => {
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <Link to="/resources/blog/" className="text-primary hover:underline">{t("backToBlog")}</Link>
        </div>
      </Layout>
    );
  }
  ...
  if (hubPost) {
    articleJsonLd.isPartOf = { "@type": "Article", name: hubPost.title, url: `https://zentroseo.com/resources/blog/${hubPost.slug}/` };
  }
  if (post.isHub && spokeArticles.length > 0) {
    articleJsonLd.hasPart = spokeArticles.map((sp) => ({ "@type": "Article", name: sp.title, url: `https://zentroseo.com/resources/blog/${sp.slug}/` }));
  }
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {hubPost && (
              <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-3">
                ...
                <span>{t("partOf")}</span>
                <Link to={`/resources/blog/${hubPost.slug}/`} className="text-primary hover:underline font-medium">{hubPost.title}</Link>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-8">
              <span className="flex items-center gap-1.5">
                ...
                {authorMember ? (
                  <Link to={`/company/team/${authorMember.authorSlug}/`} className="hover:text-primary transition-colors">{post.author}</Link>
                ) : post.author}
              </span>
              <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> {formattedDate}</span>
              {formattedModifiedDate && (
                <span className="flex items-center gap-1.5 text-primary font-medium">{t("updated")} {formattedModifiedDate}</span>
              )}
        ...
        <aside className="py-12 bg-accent/10">
          <div className="container mx-auto px-4 max-w-3xl">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              {t("moreArticlesIn")} <Link to={`/resources/blog/${hubPost!.slug}/`} className="text-primary hover:underline font-medium">{hubPost!.title}</Link> {t("series")}
            </p>
            <div className="grid sm:grid-cols-2 gap-4">
              {siblingSpokes.map((sp) => (
                <Link key={sp.slug} to={`/resources/blog/${sp.slug}/`} className="flex items-start gap-3 p-4 rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-card transition-all">
                  <BookOpen className="w-5 h-5 text-primary mt-0.5 shrink-0" />
                  <div>
                    <h3 className="font-display font-semibold text-sm line-clamp-2">{sp.title}</h3>
                    <p className="text-xs text-muted-foreground mt-1">{sp.readTime} {t("read")}</p>
                  </div>
          ...
          <div className="container mx-auto px-4 max-w-3xl">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {relatedFeatureData.map((feat) => {
                ...
                return (
                  <Link key={feat.slug} to={`/features/${feat.slug}/`} className="flex items-start gap-3 p-4 rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-card transition-all">
          ...
          <div className="container mx-auto px-4 max-w-3xl">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {relatedPillarPosts.map((rp) => (
                <Link key={rp.slug} to={`/resources/blog/${rp.slug}/`} className="flex flex-col rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-card transition-all overflow-hidden">
