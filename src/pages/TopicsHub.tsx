...
const TopicsHub = () => {
  ...
  const translatedTopics = getTranslatedTopics();

  const itemListJsonLd = {
    "@context": "https://schema.org", "@type": "ItemList", name: t("topicsHeroTitle"),
    numberOfItems: translatedTopics.length,
    itemListElement: translatedTopics.map((tp, i) => ({ "@type": "ListItem", position: i + 1, name: tp.name, url: getCanonicalUrl(lang, `/resources/topics/${tp.slug}/`) })),
  };
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <link rel="canonical" href={getCanonicalUrl(lang, "/resources/topics/")} />
        <meta property="og:title" content={t("topicsMetaTitle")} />
        <meta property="og:description" content={t("topicsMetaDesc")} />
        <meta property="og:url" content={getCanonicalUrl(lang, "/resources/topics/")} />
      ...
      <section className="py-16 md:py-20 bg-background">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="grid sm:grid-cols-2 gap-6">
            {translatedTopics.map((tp, i) => (
              <motion.div key={tp.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
                <LocalizedLink to={`/resources/topics/${tp.slug}/`} className="group block p-6 rounded-xl border border-border hover:border-primary/30 hover:shadow-card transition-all h-full">
